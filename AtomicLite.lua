...
getgenv().EvolutionLite = {
    ...
    ["Resolver"] = {
        ...
        ["Method"] = "Atomic", -- // Delta, Move Direction, No Prediction, Recalculate
    },
...
getgenv().notify = function(txt) 
    if EvolutionLite.Options["Allow Notifications"] then
        if string.lower(EvolutionLite.Options["Notifcation Type"]) == "Atomic" then 
            gggg({
                Description = txt;
                Title = "Notification";
                Duration = EvolutionLite.Options["Notifcation Duration"];
            });
        elseif string.lower(EvolutionLite.Options["Notifcation Type"]) == "Atomic" then
...
RunService.Heartbeat:Connect(function()
    ...
    if EvolutionLite.Resolver.Enabled and EvolutionLite["Silent Aim"].Enabled then 
        if EvolutionLite.Resolver.Method == "Atomic" then 
            if SilentTarget ~= nil then 
            ...
            if EvolutionLite["Silent Aim"].Enabled then 
                if SilentTarget ~= nil then
                    ...
                    if resolver then 
                        ...
                        elseif EvolutionLite.Resolver.Method == "Atomic" or  EvolutionLite.Resolver.Method == "Recalculate" then 
...
Mark.Text = "Atomic v"..utility.Version.." | .gg/"..utility.Invite
...
notify("AtomicWare No GUI V1 has loaded.")
